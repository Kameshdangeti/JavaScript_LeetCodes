
//Given an array of functions [f1,f2,f3,....,fn],return a new function fn
//that is the function composition of the array of functions.

//The functions composition of [f(x),g(x),h(x)] is fn(x)=
//f(g(x))

//The function composition of an empty list of functions is the identify
//function f(x) = x.

//You may assume each function in the array accepts one integer as input
//and returns one integer as input and returns one integer as output.

/*Input: functions = [x => x + 1, x => x * x, x => 2 * x], x = 4
Output: 65
Explanation:
Evaluating from right to left ...
Starting with x = 4.
2 * (4) = 8
(8) * (8) = 64
(64) + 1 = 65*/

var compose = function (functions) {
    if (functions.length === 0) {
        return function (x) {
            return x;
        };
    }
    return functions.reduceRight(function (prevFn, nextFn) {
        return function (x) {
            return nextFn(prevFn(x));
        };
    });
};
const fn = compose([x => x + 1, x => x * x, x => 2 * x])
console.log(fn(4));